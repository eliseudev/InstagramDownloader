...
export const formatResponse = (postID: string, json: any) => {
  ...
  if (videoList.length === 0) {
    throw Error("Esta postagem não contém nenhum vídeo");
  }
...
export const fetchPostJson = async (postID: string) => {
  ...
  if (jsonElement.length === 0) {
    throw Error(`Esse post não existe ou é privado`);
  }
...
export const getPostId = (postUrl: string) => {
  ...
  if (!postId) {
    const error = new Error("Instagram post/reel ID não foi encontrado");
    throw error;
  }
